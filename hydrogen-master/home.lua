...
if cc==nil then
  双按钮对话框("注意","该软件仅供交流学习，严禁用于商业用途，请于下载后的24小时内卸载","登录","知道了",function()
    ...
    关闭对话框(an) 跳转页面("login") end,function()
    activity.setSharedData("第一次提示","x")
    关闭对话框(an)
  end)
end
...
function 日报刷新()
  ...
  Http.get(链接,function(code,content)
    ...
    yuxun_adpqy=LuaAdapter(activity,datas3,itemc3)
  ...
  Http.get(posturl,head,function(code,content)
    if code==200 then
      ...
      for k,v in ipairs(data.data) do
        ...
        if v.type=="feed" then
         elseif v.type=="feed_advert" then
          return
        end
    ...
    if intent:find("zhihu://") then
      if intent:find "answers" then
        ...
        get:getAnswer(id,function(s)
          activity.newActivity("answer",{s.question.id,id})
        end)
