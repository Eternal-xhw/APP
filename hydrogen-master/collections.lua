...
if collections_url=="local" then
  ...
  :setresultfunc(function(tab)
    ...
    yuxunnn_ay.add{
      cavoteup=tointeger(tab.voteup_count).."",
      cacomment=tointeger(tab.comment_count).."",
      cid=tointeger(tab.question.id).."分割"..tointeger(tab.id),
      caart=tab.excerpt,
      catitle=tab.question.title,
  ...
  list5.setOnItemClickListener(AdapterView.OnItemClickListener{
    onItemClick=function(parent,v,pos,id)
      ...
      if open=="false" then
        activity.newActivity("answer",{tostring(v.Tag.cid.Text):match("(.+)分割"),tostring(v.Tag.cid.Text):match("分割(.+)")})
       else
