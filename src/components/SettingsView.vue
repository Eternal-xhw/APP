...
const loadVersionInfo = async () => {
  try {
    ...
    if (res.ok) {
      ...
      buildTime.value = cfg.builtAt
        ? new Date(cfg.builtAt).toLocaleString("zh-CN")
        : "";
...
const checkForUpdate = async () => {
  ...
  f7.preloader.show();
  try {
    const res = await fetch(basePath + "version.json?ts=" + Date.now(), {
      cache: "no-cache",
    });
    if (!res.ok) {
...
const openCitySettings = async () => {
  ...
  try {
    const res = await $http.get(
      "https://api.zhihu.com/feed-root/sections/cityList",
    );
...
const saveCity = async () => {
  ...
  try {
    await $http.post(
      "https://api.zhihu.com/feed-root/sections/saveUserCity",
...
watch(
  [homeTabs, defaultTab, defaultFollowingTab],
  () => {
    saveSettings();
  },
  { deep: true },
  ...
  try {
    ...
    if (stored) {
      ...
      if (config.monochrome !== undefined)
        isMonochrome.value = config.monochrome;
...
<template>
  <f7-page name="settings">
    ...
    <f7-list strong inset>
      ...
      >
        <select v-model="defaultFollowingTab">
          <option
            v-for="opt in followingCheckOptions"
            :key="opt.value"
            :value="opt.value"
          >
            {{ opt.label }}
    ...
    <f7-block strong inset>
      ...
      <div
        v-if="isCustomColorEnabled"
        class="display-flex align-items-center margin-top-half"
      >
        <div
          class="custom-color-picker-target margin-right"
          style="
            width: 28px;
            height: 28px;
            border-radius: 4px;
    ...
    <f7-list strong inset>
      ...
      <f7-list-item title="深色模式" :disabled="isFollowSystem">
        <template #after>
          <f7-toggle
            :checked="isDarkMode"
            @change="setDarkMode($event.target.checked)"
    ...
    <f7-list strong inset>
      <f7-list-item
        title="Fabric 项目演示"
        link="/fabric-practice"
      ></f7-list-item>
      <f7-list-item
        title="数盟ID获取教程"
        link="#"
        @click="openMsidTutorial"
      ></f7-list-item>
    ...
    <f7-list strong inset>
      <f7-list-item
        @click="$openLink('https://zhihulite.github.io/')"
        title="关于"
        :after="`构造日期 ${buildTime || '开发版'}`"
      ></f7-list-item>
      ...
      <f7-page>
        ...
        <f7-block>
          <div class="dialog-input-container">
            <p class="no-margin-top margin-bottom-half text-color-gray">
              请输入城市名称（如：北京）
            </p>
            <f7-input
              type="text"
    ...
    <f7-popup
      :opened="showTabsPopup"
      class="tabs-popup"
      @popup:closed="showTabsPopup = false"
    >
      <f7-page>
        ...
        >
          <f7-list-item
            ...
            @change="setDefaultTab(tab.id)"
            name="default_tab_radios"
          >
            <template #after>
              <f7-link
                icon-f7="minus_circle_fill"
                icon-color="red"
                @click.stop="addToDisabled(tab)"
    ...
    <f7-popup
      :opened="showMsidTutorial"
      @popup:closed="showMsidTutorial = false"
      swipe-to-close
    >
      <f7-page>
        ...
        <f7-block>
          <p>
            ...
            <f7-link
              external
              href="https://github.com/Katana-Official/SPatch-Update/releases"
              >https://github.com/Katana-Official/SPatch-Update/releases</f7-link
            >
          </p>
          <p>
            安装mt管理器和知乎 打开分身的知乎同意协议进入主页后 退出打开mt管理器
            进入侧滑 点击终端执行器 执行以下命令
          </p>
          <f7-block
            strong
          ...
          >
            ...
            | sed 's/.*&lt;string
            name="device_id"&gt;\([^&lt;]*\)&lt;\/string&gt;.*/\1/'
          </f7-block>
          <p>
            如果是root 就不用这一步 直接打开mt管理器 输入su后将上述指令的
            top.bienvenido.saas.i18n/app_data_anon/ 替换为空 执行即可得到参数
          </p>
          <p>将输出文本复制粘贴到这里即可</p>
          <p>
            或者去github.com或其他途径搜索 x-ms-id 将他的输出参数复制即可
            留空则为使用默认ms-id
          </p>
        </f7-block>
      </f7-page>
